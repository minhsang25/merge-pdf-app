...
async function fileToPDFBuffer(file) {
  ...
  switch (fileType) {
    ...
    default:
      throw new Error(
        `Không thể chuyển đổi file ${file.name} sang PDF. Định dạng không được hỗ trợ.`
      );
  }
}
...
function renderFileList() {
  ...
  uploadedFiles.forEach((file, index) => {
    ...
    const fileType = getFileType(file);
    tdType.textContent = fileType.toUpperCase();
    ...
    if (convertedPDFs.has(file.name)) {
      statusElement = createStatusElement("converted", "Đã chuyển đổi");
    } else {
      statusElement = createStatusElement("converting", "Đang xử lý...");
...
mergeBtn.addEventListener("click", async () => {
  const selectedFiles = uploadedFiles.filter(
    (_, index) => document.getElementById(`file_${index}`).checked
  );
